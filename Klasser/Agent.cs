class Agent
{
  ...
  public Agent()
    ...
    public void skjuter(Villan target)
    {
        Random x = new Random();
        int rInt = x.Next(0, skott+1);
        Console.WriteLine($"{namn} skjuter {rInt} skott");
        
        
         target.Hälsa -= rInt;
        Console.WriteLine("Skotten träffar "+ target.Namn+ " Vilket gör "+ rInt+ " skada."+ " Nu har "+ target.Namn +" "+ target.Hälsa+ " i hälsa.");
        if (target.Hälsa <= 0)
        {
